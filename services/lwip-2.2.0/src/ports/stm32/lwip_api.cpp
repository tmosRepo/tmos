...
err_t lwip_cbf_sent(void *arg, struct tcp_pcb *pcb, u16_t len)
{
	...
	LWIP_DEBUGF(LWIP_PORT_API,("LWIP sent "));
	if(client == NULL) {
		LWIP_DEBUGF(LWIP_PORT_API,("NULL socket!\n"));
		return (ERR_VAL);
	}
	LWIP_DEBUGF(LWIP_PORT_API,("sock: %u/%x len %u\n", client->sock_id, client->sock_state, len));
//	client->dst.as_byteptr += len;
	...
	if(client->mode1 == TCPHS_WRITING)
	{
		...
		{
			LWIP_DEBUGF(LWIP_PORT_API,("LWIP sent ACK %u\n", client->src.as_int - client->dst.as_int));
			return (ERR_OK);
		}
		len = client->len;
		if(len)
...
err_t lwip_cbf_recv(void *arg, struct tcp_pcb *pcb, struct pbuf *p, err_t err)
{
	...
	LWIP_DEBUGF(LWIP_PORT_API,("LWIP recv "));
	if(p)
	{
		LWIP_DEBUGF(LWIP_PORT_API,(" len=%d tot=%d", p->len, p->tot_len));
	}

	if(client && client->sock_id < LWIP_TCP_PCBS_CNT)
	{
		LWIP_DEBUGF(LWIP_PORT_API,("socket %u %x\n", client->sock_id, err));
		lwip_sock_data_t* sock_data = &g_lwip_socks[client->sock_id];
...
#if LWIP_UDP_PCBS_CNT
void lwip_cbf_udp_recv(void *arg, struct udp_pcb *pcb, struct pbuf *p, const ip_addr_t *addr, u16_t port)
{
	CSocket* client = (CSocket*)arg;

	LWIP_DEBUGF(LWIP_PORT_API,("LWIP UDP recv %x", client));
	if(p)
	{
		LWIP_DEBUGF(LWIP_PORT_API,(" len=%d tot=%d", p->len, p->tot_len));
...
void lwip_cbf_err(void *arg, err_t err)
{
	...
	LWIP_DEBUGF(LWIP_PORT_API,("TCP ERR %x for %x", err, client));
	if(client && client->sock_id < LWIP_TCP_PCBS_CNT)
	{
		//send signal
...
static RES_CODE api_close_internal(CSocket* client, unsigned int rxtx, struct netif *netif)
{
	...
	if(sock_mode->mode.sock_type == IP_SOCKET_TCP)
	{
		...
		if(pcb && client->mode1)
		{
			...
			if (pcb->state == LISTEN)
			{
				((tcp_pcb_listen *)pcb)->accept = NULL;
			} else
			{
...
err_t lwip_cbf_connected(void *arg, struct tcp_pcb *pcb, err_t err)
{
	...
	LWIP_DEBUGF(LWIP_PORT_API,("LWIP connect %x", client));
	if(client == NULL)
		return (ERR_VAL);
...
void lwip_finish_dns(CSocket* client, const ip_addr_t *ipaddr)
{
	RES_CODE res = RES_SIG_ERROR;
	lwip_mode_t* sock_mode = (lwip_mode_t*)client->mode.as_voidptr;
 ...
 */
void lwip_cbf_dns(const char *name, const ip_addr_t *ipaddr, void *arg)
{
	CSocket* client = (CSocket*)arg;
...
err_t lwip_cbf_accept(void *arg, struct tcp_pcb *newpcb, err_t err)
{
	...
	LWIP_DEBUGF(LWIP_PORT_API,("LWIP accept %x", client));
	if(client == NULL)
		return (ERR_VAL);
	...
	if(client->sock_id < LWIP_TCP_PCBS_CNT)
	{
		...
		if(sock_data->pcb && (client->mode1 == TCPHS_BIND) )
		{
			...
			if(pcb)
			{
				...
				pcb->callback_arg = client;
				((tcp_pcb_listen *)pcb)->accept = lwip_cbf_accept;
